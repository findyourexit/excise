...
Register-ArgumentCompleter -Native -CommandName 'excise' -ScriptBlock {
    ...
    $completions = @(switch ($command) {
        'excise' {
            ...
            [CompletionResult]::new('--event-buffer', '--event-buffer', [CompletionResultType]::ParameterName, 'Maximum queued worker events (16 to 4096)')
            [CompletionResult]::new('--exclude', '--exclude', [CompletionResultType]::ParameterName, 'Ordered gitignore-style exclusion pattern')
            [CompletionResult]::new('--memory-mib', '--memory-mib', [CompletionResultType]::ParameterName, 'Whole-process memory limit in MiB')
            [CompletionResult]::new('--temporary-storage-mib', '--temporary-storage-mib', [CompletionResultType]::ParameterName, 'Directory-plan and deletion-result storage limit per session (at least 2 MiB)')
            [CompletionResult]::new('--scan-store-mib', '--scan-store-mib', [CompletionResultType]::ParameterName, 'Private scan data and page-index storage limit per session (at least 2 MiB, capped by scratch-volume free space)')
            [CompletionResult]::new('--scan-store-dir', '--scan-store-dir', [CompletionResultType]::ParameterName, 'Parent directory for the private scan-storage session')
            [CompletionResult]::new('--scan-store-reserve-mib', '--scan-store-reserve-mib', [CompletionResultType]::ParameterName, 'Scratch space kept outside scan-storage files (defaults to 25 percent of free space)')
            [CompletionResult]::new('--theme', '--theme', [CompletionResultType]::ParameterName, 'Built-in color theme')
            [CompletionResult]::new('--keymap', '--keymap', [CompletionResultType]::ParameterName, 'Keyboard preset. Arrows and safety keys always work')
            [CompletionResult]::new('--format', '--format', [CompletionResultType]::ParameterName, 'Output mode. Table and JSON modes do not require a terminal')
