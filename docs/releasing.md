...
| JSON reports | Keep each published `scan-report`, `deletion-history`, and `native-path` version stable. Add fields only when consumers can ignore them. Increase the format version for incompatible changes. | JSON is the machine-readable format. Table layout is not stable. |
