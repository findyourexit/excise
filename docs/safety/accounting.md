...
Each session has a 4 GiB temporary-storage budget and an adaptive scan-store budget. By default the scan store receives 75 percent of free space on the volume that hosts its private session directory, preserving the other 25 percent; users may set an upper bound with `--scan-store-mib` or choose a different reserve with `--scan-store-reserve-mib`. It covers the scanner journal, ScanStore runs, and the bounded build index used to publish direct-child pages. The temporary-storage budget covers overflow directory deletion-plan and outcome records. Each budget grows only as its owned files are written. Page publication consumes raw fact runs phase by phase and retains only its compact child-query run, avoiding a second full-generation scan-store reservation after publication. Every owned file reserves capacity before it grows and releases it only after shrinking or cleanup.
