...
Opening a directory while the initial scan is active, or opening a summarized directory at any time, enters this scan field before its staging model is prepared. The focused scan prefers one-third of the model budget and traverses serially, so opening time cannot inflate its capacity; its completed result remains authoritative over later primary-scan events for that path. If compaction cannot free that preferred slice, it uses the remaining capacity down to an 8 MiB floor. Below that floor, Excise keeps the partial folder open and the primary scan continues without a blocking notice. Capacity preparation yields between frames, and `Esc` cancels the focused refresh while returning to the parent folder.
