...
The scanner uses a fixed number of workers and walks directories without recursion. Directory tasks and worker events use queues with fixed limits. The scheduler publishes one coalesced, bounded status snapshot rather than a progress-event backlog, so observing pending, active, and terminal work never delays scan output. The scanner journal, ScanStore runs, and the bounded build index used to publish direct-child pages use an independent scan-store budget; directory deletion-plan and outcome records use the temporary-storage budget. Both reserve capacity before their files grow. A scanner-journal capacity breach reports an actionable incomplete scan; a ScanStore write, merge, or page-index failure is never published as an exact generation; a deletion plan that cannot retain every reviewed identity is rejected before mutation.
...
The interactive map materializes one 512-entry direct-child page plus its ancestor chain, then retains only a fixed small LRU cache of recently visited immutable pages. `PageDown` and `PageUp` move between concrete pages; no child is collapsed into an undeletable storage aggregate. An unfiltered cache miss seeks directly into the publication-time `ChildQuery` run and decodes only the requested slice, so completed-folder navigation has no loading view or live-model fallback. Filters walk only the scoped canonical subtree in bounded page memory: a matching descendant retains its direct ancestor at each visible level, so users can drill to the match without reviving the mutable arena. Entering or leaving a folder never launches a focused scan; only a generation-invalidating mutation may schedule a root rebuild. Page entries retain the scan-time identity, allocation, and modification snapshot used by deletion planning.

### Working Model

Scanner workers seal canonical path and identity facts before the owner observes their UI events. During scanning the UI keeps only bounded progress and interaction state; it does not retain a second path arena whose capacity could change the scan result. Once a ScanStore generation publishes, the interactive map and scan reports read its immutable canonical pages and query records: names, metrics, and child lists are bounded to the visible page rather than retained for the complete filesystem. Walking and removal use loops rather than the call stack.
