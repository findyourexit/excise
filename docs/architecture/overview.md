...
A single scanner service executes the initial walk and invalidation-triggered root rebuilds serially. Its one-slot rebuild command lane and per-rebuild cancellation flag preserve the current published generation until a strictly newer canonical generation completes; focus requests only reprioritize work in the active generation.
