...
The interactive `t` picker previews these existing `runtime.theme` values without changing configuration. Pressing `Enter` selects the preview for the current session; it is written only through the existing save-preferences exit choice, and `Esc` restores the original value.
